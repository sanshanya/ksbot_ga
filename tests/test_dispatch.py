...
def test_same_chat_queue_is_serial() -> None:
    ...
    service._queues, service._active_chats, service._active_requesters = {}, set(), {}
...
def test_stop_targets_active_requester_and_clears_queue(tmp_path) -> None:
    actions = []
    service = GaWpsService.__new__(GaWpsService)
    service._stopped = threading.Event()
    ...
    service.approvals = SimpleNamespace(
        cancel_chat=lambda chat, user: actions.append((chat, user)),
    ...
    service._active_requesters = {"chat-1": "owner"}
    service.on_message(message("/stop"))
    assert actions == [("chat-1", "owner"), "abort"]
